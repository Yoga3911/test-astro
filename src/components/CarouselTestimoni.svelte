...
<section id="testimoni-alumni" class="py-16 sm:py-24 bg-white relative overflow-hidden">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    
    <!-- Section Header (Ala SDH: FEATURING OUTSTANDING ALUMNI) -->
    <div class="text-center max-w-3xl mx-auto mb-14">
      <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-100 text-amber-800 text-xs font-extrabold uppercase tracking-widest mb-3">
    ...
    <div class="max-w-4xl mx-auto bg-gradient-to-br from-slate-50 to-blue-50/40 rounded-3xl p-6 sm:p-10 md:p-14 border border-slate-200/90 shadow-xl shadow-blue-900/5 relative">
      
      <!-- Big Quote Mark Background -->
      <div class="absolute top-6 right-8 text-blue-200/50 font-serif text-8xl md:text-9xl select-none pointer-events-none leading-none">
        ”
      </div>
        ...
        {#if currentIndex === index}
          <div class="animate-fadeIn relative z-10 flex flex-col md:flex-row gap-8 items-center md:items-start">
            ...
            <div class="flex flex-col items-center text-center shrink-0 w-full md:w-56">
              <div class="w-28 h-28 sm:w-36 sm:h-36 rounded-3xl overflow-hidden shadow-lg border-4 border-white mb-4 bg-slate-200">
                <img
                  src={item.photo}
                  alt={item.name}
              ...
              <span class="inline-block mt-1 px-3 py-0.5 rounded-full bg-blue-100 text-blue-800 text-xs font-bold">
                {item.batch}
              </span>
            ...
            <div class="flex-1 space-y-4">
              <h3 class="text-xl sm:text-2xl font-heading font-extrabold text-blue-950 leading-snug border-b border-blue-100 pb-3">
                "{item.headline}"
              </h3>
      ...
      <div class="flex items-center justify-between mt-10 pt-6 border-t border-slate-200/80">
        ...
        <button
          ...
          class="flex items-center gap-2 px-4 py-2 rounded-xl text-slate-700 hover:text-blue-700 bg-white hover:bg-slate-100 border border-slate-200 text-sm font-bold shadow-xs active:scale-95 transition-all cursor-pointer"
          onclick={prev}
        >
          <span>←</span>
