...
>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="text-center max-w-3xl mx-auto mb-14">
      <div
        class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-100 text-amber-800 text-xs font-extrabold uppercase tracking-widest mb-3 border border-amber-200 shadow-xs"
      >
        <span>FEATURING OUTSTANDING ALUMNI</span>
      </div>
      <h2
        class="text-3xl sm:text-4xl font-heading font-black text-slate-900 tracking-tight"
      >
    ...
    <div
      class="max-w-4xl mx-auto bg-gradient-to-br from-white via-blue-50/30 to-slate-50 rounded-3xl p-6 sm:p-10 md:p-14 border border-slate-200/90 shadow-xl shadow-blue-900/5 relative"
    >
      ...
      {#each ALUMNI_TESTIMONIALS as item, index}
        {#if currentIndex === index}
          <div
            class="animate-fadeIn relative z-10 flex flex-col md:flex-row gap-8 items-center md:items-start"
          >
            <!-- Alumni Avatar & Quick Profile Card -->
            <div
              class="flex flex-col items-center text-center shrink-0 w-full md:w-56"
            >
              <div
                class="w-28 h-28 sm:w-36 sm:h-36 rounded-3xl overflow-hidden shadow-lg border-4 border-white mb-4 bg-slate-200 ring-1 ring-slate-200"
              >
                <img
                  src={item.photo}
                  alt={item.name}
      ...
      <div
        class="flex items-center justify-between mt-10 pt-6 border-t border-slate-200/80"
      >
        <!-- Prev Button -->
        <button
          type="button"
